...
namespace GCS_CPC_EMEC.Services
{
    public class Aes  // Advanced Encryption Standard
    {
        ...
        public void Cipher(byte[] input, ref byte[] output)  // encipher 16-bit input
        {
            // state = input
        ...
        public void InvCipher(byte[] input, ref byte[] output)  // decipher 16-bit input
        {
            // state = input
            this.State = new byte[4, Nb];  // always [4,4]
            for (int i = 0; i < (4 * Nb); ++i)
    ...
    /*f*/  {0x17, 0x2b, 0x04, 0x7e, 0xba, 0x77, 0xd6, 0x26, 0xe1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0c, 0x7d} };
        ...
        public void encrypt(byte[] input, ref byte[] output)
        {
            byte[] arrin = new byte[16];
            byte[] arrout = new byte[16];
            for (int i = 0; i < input.Length; i += 16)
            {
                Buffer.BlockCopy(input, i, arrin, 0, 16);
                Cipher(arrin, ref arrout);
                Array.Resize(ref output, i + 16);
                Buffer.BlockCopy(arrout, 0, output, i, 16);
            }
        }
        public void decrypt(byte[] input, ref byte[] output)
        {
            byte[] arrin = new byte[16];
            byte[] arrout = new byte[16];
            for (int i = 0; i < input.Length; i += 16)
            {
                Buffer.BlockCopy(input, i, arrin, 0, 16);
                InvCipher(arrin, ref arrout);
                Array.Resize(ref output, i + 16);
                Buffer.BlockCopy(arrout, 0, output, i, 16);
            }
        }
        public void resize2AES128(ref byte[] sendbytes)
        {
            int num = sendbytes.Length % 16;
            if (num != 0)
            {
                Array.Resize(ref sendbytes, sendbytes.Length + (16 - num));
            }
        }
    }  // class Aes
    public static class clsGen
    {
        public static void resize2AES128(ref byte[] sendbytes)
        {
            int numbyteadd = sendbytes.Length % 16;
            if (numbyteadd != 0)
            {
                Array.Resize(ref sendbytes, sendbytes.Length + (16 - numbyteadd));
            }
        }
    }
}
